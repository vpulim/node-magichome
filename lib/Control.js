...
const COMMAND_TIMEOUT = 1000; // 1 sec
...
class Control {
	...
	_sendCommand(buf, expect_reply, callback) {
		...
		if(this._commandQueue.length == 0 && this._socket == null) {
			this._commandQueue.push({callback: callback, expect_reply: expect_reply, ts: Date.now()});
			...
			if (Date.now() - this._commandQueue[0].ts > COMMAND_TIMEOUT) {
			  while (this._commandQueue[0]) {
			    var timedout_command = this._commandQueue.shift();
			    var cb = timedout_command.callback;
			    if(cb != undefined) {
	...
	setBrightness(brightness, speed, callback) {
		brightness = clamp(brightness, 0, 100);
		speed = clamp(speed, 0, 100);
		var level = Math.round(255 / 100 * brightness)
		var delay = speedToDelay(speed);

		var cmd_buf = Buffer.from([0x31, level, 0x00, 0x00, delay, this._characteristics.set_color_magic_bytes[0], this._characteristics.set_color_magic_bytes[1]]);
